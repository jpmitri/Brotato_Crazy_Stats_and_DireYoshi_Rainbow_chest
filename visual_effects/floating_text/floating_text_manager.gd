...
func on_stat_added(stat:String, value:int, pos_sounds:Array = stat_pos_sounds, neg_sounds:Array = stat_neg_sounds)->void :
	display_icon(value, ItemService.get_stat_icon(stat), pos_sounds, neg_sounds)


func display_icon(value:int, icon:Resource, pos_sounds:Array = [], neg_sounds:Array = [], pos:Vector2 = player.global_position - Vector2(0, 50), p_direction:Vector2 = direction)->void :
	if value > 0:
		display("+" + str(value), pos, Utils.GOLD_COLOR, icon, duration * 2, false, p_direction)
		
		if pos_sounds.size() > 0:
			SoundManager.play(Utils.get_rand_element(pos_sounds), - 3, 0.2, true)
	else :
		display(str(value), pos, Color.red, icon, duration * 2, false, p_direction)
		
		if neg_sounds.size() > 0:
			SoundManager.play(Utils.get_rand_element(neg_sounds), - 8, 0.2, true)
...
func _on_unit_took_damage(unit:Unit, value:int, _knockback_direction:Vector2, _knockback_amount:float, is_crit:bool, is_miss:bool, _effect_scale:float, hit_type:int)->void :
