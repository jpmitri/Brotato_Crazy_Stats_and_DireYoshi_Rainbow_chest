...
func init_base_stats(from_stats:WeaponStats, weapon_id:String = "", weapon_classes:Array = [], effects:Array = [], is_structure:bool = false)->WeaponStats:
	...
	new_stats.max_range = max(MIN_RANGE, base_stats.max_range + Utils.get_stat("stat_range")) as int
	...
	new_stats.damage += get_scaling_stats_value(base_stats.scaling_stats)
...
func explode(effect:Effect, pos:Vector2, damage:int, accuracy:float, crit_chance:float, crit_dmg:float, burning_data:BurningData, is_healing:bool = false, ignored_objects:Array = [])->void :
	var main = get_tree().current_scene
