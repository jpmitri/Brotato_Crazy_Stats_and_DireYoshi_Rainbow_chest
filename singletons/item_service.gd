...
export (Resource) var rainbow_box = null
...
func get_rand_item_from_wave(wave:int, type:int, shop_items:Array = [], prev_shop_items:Array = [], fixed_tier:int = - 1)->ItemParentData:
				...
				if type == TierData.WEAPONS:
								...
								if RunData.weapons.size() > 0:
												...
												elif randf() < chance_same_weapon_class:
				...
				elif type == TierData.ITEMS and Utils.is_chance_factor(ProgressData.settings.shop_tag_chance) and RunData.current_character.wanted_tags.size() > 0:
...
func get_value(wave:int, base_value:int, affected_by_items_price_stat:bool = true)->int:
				...
				if ProgressData.settings.remove_inflation:
								return max(1.0, ((base_value + (base_value * (0.1 + diff_factor))) * items_price_factor)) as int
				else :
								return max(1.0, ((base_value + wave + (base_value * wave * (0.1 + diff_factor))) * items_price_factor)) as int


func get_reroll_price(wave:int, last_reroll_value:int)->int:
				if ProgressData.settings.fixed_reroll:
								return last_reroll_value + 5 as int
				else :
								return max(1.0, last_reroll_value + max(1.0, 0.5 * wave)) as int


func get_recycling_value(wave:int, from_value:int)->int:
				return max(1.0, (get_value(wave, from_value) * clamp((0.25 + (RunData.effects["recycling_gains"] / 100.0)), 0.01, 1.0))) as int
