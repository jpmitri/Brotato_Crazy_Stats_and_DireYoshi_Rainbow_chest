...
func _ready()->void :
				...
				if RunData.resumed_from_state:
								...
								if _free_rerolls > 0:
												...
												if ProgressData.settings.fixed_reroll:
																_last_reroll_price = 5
												else :
																_last_reroll_price = ItemService.get_reroll_price(0, RunData.current_wave - 1)
								else :
												if ProgressData.settings.fixed_reroll:
																_last_reroll_price = 5
																_reroll_price = 5
				...
				_items_container.set_data("ITEMS", Category.ITEM, RunData.items, true)
...
func on_shop_item_bought(shop_item:ShopItem)->void :
				...
				if _shop_items.size() == 0:
								if ProgressData.settings.fixed_reroll:
												_last_reroll_price = _reroll_price - 5
								else :
												_last_reroll_price = _reroll_price
								_reroll_price = 0
								_reroll_button.init(_reroll_price)
				elif has_new_rerolls:
								if not ProgressData.settings.fixed_reroll:
												_last_reroll_price = _reroll_price
								_reroll_price = 0
								_free_rerolls -= 1
								_reroll_button.init(_reroll_price)
...
func on_item_combine_button_pressed(weapon_data:WeaponData)->void :
				_focus_manager.reset_focus()
				
				_weapons_container._elements.remove_element(weapon_data, 2)
				RunData.remove_weapon(weapon_data)
				RunData.remove_weapon(weapon_data)
				
				reset_item_popup_focus()
				
				RunData.add_weapon(weapon_data.upgrades_into)
				
				_stats_container.update_stats()
				
				_weapons_container._elements.add_element(weapon_data.upgrades_into)
				
				if Input.get_mouse_mode() == Input.MOUSE_MODE_HIDDEN:
								_weapons_container._elements.focus_element(weapon_data.upgrades_into)
...
func on_item_discard_button_pressed(weapon_data:WeaponData)->void :
				...
				RunData.remove_weapon(weapon_data)
				RunData.add_gold(ItemService.get_recycling_value(RunData.current_wave, weapon_data.value))
				reset_item_popup_focus()
				_shop_items_container.update_buttons_color()
